package codes.pepper.whimsicalart.feature.editor.ui.frames
...
fun FramePreview(
    ...
    Canvas(modifier = modifier.fillMaxSize()) {
        ...
        val inset = stroke / 2f
        if (frame.hasShadow) {
            val shadow = Color(frame.shadowColor).copy(alpha = 0.35f)
            val spx = stroke * 0.4f
            drawRoundRect(
                color = shadow,
                topLeft = Offset(spx, spx),
                size = Size(size.width - 2f * spx, size.height - 2f * spx),
