...
class SkinDenoiseProcessorTest {
    ...
    @Test
    fun `denoise preserves transparency instead of filling the background black`() {
        // A cut-out: transparent background, opaque subject.
        val bitmap = Bitmap.createBitmap(128, 128, Bitmap.Config.ARGB_8888)
        for (y in 0 until 128) {
            for (x in 0 until 128) {
                val subject = x in 32..96 && y in 32..96
                bitmap.setPixel(x, y, if (subject) Color.argb(255, 120, 90, 60) else Color.TRANSPARENT)
            }
        }

        val out = SkinDenoiseProcessor.denoise(bitmap, 0.5f)

        // The transparent background stays transparent (not opaque black).
        assertEquals(0, Color.alpha(out.getPixel(8, 8)))
        // The opaque subject stays opaque, with its RGB intact on a flat colour.
        assertEquals(255, Color.alpha(out.getPixel(64, 64)))
        assertEquals(Color.argb(255, 120, 90, 60), out.getPixel(64, 64))
    }
